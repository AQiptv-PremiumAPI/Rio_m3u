...
  const url = r.url || '';
  const id = url.split(/\/(?:key|mpd|key1|mpd1|key2|mpd2|key1bb|mpd1bbn)\//).pop().split('?')[0];
  const u = (process.env.USER_ID || '').split(',').map(e => e.split(':')).find(([n]) => n === name);
...
    return s.status(401).send('Unauthorized Access, Telegram:@RioUniverse');
...
    if (!validIDs.includes(id)) return s.status(403).send('Incorrect ID, Telegram:@RioUniverse');
